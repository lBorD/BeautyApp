...
Na Agenda, os cards devem priorizar horário, status, cliente, serviço e sincronização Google. Ações secundárias ficam recolhidas sob um acionador compacto de “mais ações”, com área de toque acessível, e aparecem em um popover inline sobre a lista. Somente um popover pode permanecer aberto. Atendimentos cancelados continuam visíveis de forma discreta e oferecem somente a ação de restauração; enquanto cancelados, não ocupam horários nem entram nos indicadores e marcadores. Confirmações abertas dentro do fluxo de agendamento devem ser inline para não empilhar modais nativos no iOS.
